...
def spawn_boss():
    ...
    # 보스 위치를 화면의 오른쪽 끝 근처로 설정
    random_x = random.randint(joystick.width - 50, joystick.width)
...
while True:
    ...
    for enemy in enemys_list:
        if enemy.boss_check:  # 적 캐릭터가 보스일 경우
            if enemy.health > 0:  # 보스가 살아있다면
                for bullet in bullets:
                    if bullet.state != 'hit' and bullet.collision_check([enemy]):
                        enemy.health -= bullet.damage  # 총알 데미지만큼 체력 감소
                        bullet.state = 'hit'  # 총알 상태를 'hit'으로 변경
                        print(f"보스 체력: {enemy.health}")
            else:
    ...
    # 게임 종료 조건 (보스가 죽었을 경우)
    if len(enemys_list) == 0 and any(enemy.state == 'die' for enemy in enemys_list):
        print("게임 종료! 보스를 처치했습니다.")
        break  # 게임 종료

    # 화면 출력 및 조이스틱 화면 갱신
    cropped_background = scroller.get_cropped_image()
    display.paste(cropped_background, (0, 0))

    for enemy in enemys_list:
        if enemy.state != 'die':
            display.paste(enemy.show_motion, (enemy.position[0], enemy.position[1]), enemy.show_motion)

    display.paste(player.show_player_motion, (player.character_x, player.character_y), player.show_player_motion)
